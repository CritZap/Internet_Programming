...
public class GetContent {
	...
	public static void main(String[] args) throws UnknownHostException, IOException {
		...
		CharacterHttpResponse response = example.createRequest(HOST_NAME, HTTP_METHOD_GET, "/index.html");
		...
		while(statusLine.indexOf("301")>0 || statusLine.indexOf("302")>0){
			for (HttpHeader next : response.getHeaders()) {
				...
				if(next.getName().indexOf("ocation") > 0){
					response = example.createRequest(next.getValue().replace("http://","").replace("/index.html", ""), HTTP_METHOD_GET,"/index.html");
